...
import (
	...
	"chast.io/core/internal/run_model/builder/refactoring"
...
type RunModelBuilder interface {
	BuildRunModel(*model.Recipe, *model2.ParsedArguments) (*model2.RunModel, error)
}

func BuildRunModel(parsedRecipe *model.Recipe, arguments []string, workingDirectory string) (*model2.RunModel, error) {
...
func getBuilder(parsedRecipe *model.Recipe) (RunModelBuilder, *model.BaseRecipe, error) {
...
func handleArguments(baseRecipe *model.BaseRecipe, args []string) (*model2.ParsedArguments, error) {
	requiredArgsCount := collection.Count(baseRecipe.Arguments, func(argument model.Argument) bool { return argument.Required })
	...
	return &model2.ParsedArguments{Arguments: argsMap, UnmappedArguments: args[len(argsMap):]}, nil
}

func verifyArgument(argument *model.Argument, value string) error {
	if argument.Type != "" {
